...
if [ $1 -eq 1 ]; then
	...
	if [ -z "$result" ];then
		...
		used_mib_id=$(uci show devlist | grep 'mib_index' | awk -F '=' '{print $2}' | tail -n 1 | sed "s/'//g")
		if [ -n "$used_mib_id" ];then
			mib_id=$((used_mib_id + 1))
		else
			mib_id=1
		fi
		uci set devlist.$mac.mib_index=$mib_id
	else
		mib_id=$(uci get devlist.$mac.mib_index)
	fi
	...
	if [ -n "$result" ];then
		ubus call dpns.l2 macmib "{\"mac\":\"$2\",\"vlan_id\":$6,\"mib_index\":$mib_id,\"mib_mode\":1,\"mib_op\":0}"
	fi
